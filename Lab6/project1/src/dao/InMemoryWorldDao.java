...
public class InMemoryWorldDao implements WorldDao {
	...
	private InMemoryWorldDao() {
		...
		// until now there's no differenece between different hashed key ABW and hamada
		// as being from the data itself or not.
		//
		// countries.put("ABW", new Country("ABW", "Aruba", "North America", 103000,
		// 193.00, 828.00, 129));
		// countries.put("hamada", new Country("ABW", "Aruba", "North America", 103000,
		// 193.00, 828.00, 129));
		...
		Map<String, String> highestPopulatedCityOfEachContinent = countries.values().stream()
				.collect(Collectors.groupingBy(Country::getContinent, // Grouping by continent
						Collectors.collectingAndThen(Collectors.mapping(
								country -> country.getCities().stream().mapToInt(City::getPopulation).max().orElse(0), // Map
																														// each
																														// continent
																														// to
																														// its
